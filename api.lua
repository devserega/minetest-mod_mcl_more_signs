...
--mcl_more_signs.standard_wood_groups = table.copy(has_default_mod and minetest.registered_items["default:sign_wall_wood"].groups or {})
--mcl_more_signs.standard_wood_groups.attached_node = nil
...
mcl_more_signs.standard_wood_sign_sounds  = table.copy(mcl_sounds.node_sound_stone_defaults() or {})
...
mcl_more_signs.wall_fdir_to_back = {
	nil,
	nil,
	{  0,  1 },
	{  0, -1 },
...
mcl_more_signs.fdir_flip_to_back = {
	[0] = {  0,  2 },
	[1] = {  2,  0 },
	[2] = {  0, -2 },
	[3] = { -2,  0 }
...
mcl_more_signs.wall_fdir_flip_to_back = {
	[2] = {  2,  0 },
	[3] = { -2,  0 },
	[4] = {  0,  2 },
	[5] = {  0, -2 },
...
-- Функция mcl_more_signs.split_lines_and_words(text) выполняет разбиение текста:
-- Сначала на строки (по символу новой строки \n),
-- Затем каждую строку на слова (по пробелам).
-- Возвращает таблицу, где каждая строка — это список слов.
...
-- 4) Является ли игрок админом (settings:get("name")).
...
function mcl_more_signs.register_yard_sign(name, def, raw_def)
	...
	--yard_def.groups.not_in_creative_inventory = 1
...
function mcl_more_signs.register_sign(name, raw_def)
	...
	--def.after_place_node = raw_def.after_place_node or mcl_more_signs.after_place_node
	...
	if raw_def.entity_info then
		...
		if raw_def.locked and not raw_def.after_place_node then
			def.after_place_node = function(pos, placer, itemstack, pointed_thing)
				mcl_more_signs.after_place_node(pos, placer, itemstack, pointed_thing, true)
			end
		end
		]]
	end
	...
	def.sounds = raw_def.sounds or mcl_more_signs.standard_wood_sign_sounds
	--def.paramtype2 = raw_def.paramtype2 or "wallmounted"
	def.on_rotate = raw_def.on_rotate or mcl_more_signs.handle_rotation
	def.on_place = raw_def.on_place or mcl_more_signs.on_place
...
minetest.register_on_mods_loaded(function()
	if not minetest.registered_privileges[mcl_more_signs.edit_priv] then
		-- commented because locked not work minetest.register_privilege("signslib_edit", {})
	end
end)
