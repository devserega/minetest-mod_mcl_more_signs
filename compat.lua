-- Этот код предназначен для совместимости с другими модами 
-- (default, cottages, prefab_redo, streetspoles, streetlamps и т. д.).

-- 1) Поддержка заборов (default)
-- *) Если мод default включён, берутся все деревянные заборы.
-- *) Для них включается check_for_pole = true — значит, они будут 
-- распознаваться как опоры (например, для установки табличек).
...
-- 2) Совместимость с cottages
-- *) Если мод cottages включён, копируется его node_box для стола (cottages:table).
-- *) Добавляется check_for_pole = true, чтобы стол можно было использовать как опору.
-- *) selection_box = cbox — сохраняет оригинальный хитбокс.
...
-- 3) Совместимость с prefab_redo
-- *) Если установлен prefab_redo, у "prefab_redo:concrete_railing" меняется selection_box (его хитбокс).
-- *) Добавляется check_for_pole = true, чтобы перила работали как опоры.
...
-- 4) Совместимость с streetspoles
-- Здесь код сложнее: он добавляет поддержку полюсов, Т-образных соединений и соединений по параметрам param2.
-- *) Определяет, какие значения param2 соответствуют разным ориентациям столбов.
-- *) Добавляет функции check_for_pole и check_for_horiz_pole, чтобы правильно определять соединения.
-- *) Позволяет табличкам, знакам и фонарям крепиться к этим полюсам.
